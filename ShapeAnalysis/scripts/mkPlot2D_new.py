    ...
    def makePlot(self, inputFile, outputDirPlots, variables, cuts, samples, plot, legend, groupplot):
        ...
        for cutName in self._cuts:
            ...
            for variableName, variable in self._variables.iteritems():
                ...
                #tcanvas = ROOT.TCanvas("c_2D_" + variableName, "c_2D_" + variableName,800, 800) #Iniciate the canvas, one for each variable.                                   
                ...
                for sampleName, plotdef in plot.iteritems():
                    ...
                    if plotdef['isData']==1:
                        continue
                    print "len_0=", len(np.array(rango[0]))
                    print "len_1 =", len(np.array(rango[1]))
                    print "np.asarray(rango[0]) = ", np.asarray(rango[0])
                    print "np.asarray(rango[1]) = ", np.asarray(rango[1])
                    print "rango_0 = ", rango[0]
                    print "rango_1 = ", rango[1]
                    np_array_x=np.array(rango[0])
                    np_array_y=np.array(rango[1])
                    # xedges[] = {0.,100.,200.,300.,500.,1000.,1500.,2000.,2500.,3000.,3500.,4000.,4500.}
                    # yedges[] = {0.,0.05, 0.10,0.15, 0.20,0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.0}
                    ...
                    histogram2D = ROOT.TH2D(sampleName+"_2DHist_"+variableName, sampleName+"_2DHist_"+variableName, len(np.asarray(rango[0]))-1, xedges, len(np.asarray(rango[1]))-1, yedges)
                    ...
                    for i in range(1, len(np.asarray(rango[0]))):
                        ...
                        for j in range(1, len(np.asarray(rango[1]))):
                            ...
                            print "histos[sampleName] = ", histos[sampleName]
                            print "i, j = ", i, j
                            print "histos[sampleName].GetBinContent()", histos[sampleName].GetBinContent((len(np.asarray(rango[0]))-1)*(i-1)+j)
                            print "", index
                    print "-------------"
